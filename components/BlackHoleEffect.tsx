...
const BlackHoleEffect: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const rotateIconSVG =
      '<svg class="rotate-icon" viewBox="0 0 24 24"><path d="M23 4v6h-6"></path><path d="M1 20v-6h6"></path><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>';
    const updateAutoRotateText = () => {
      if (autoRotateToggle)
        autoRotateToggle.innerHTML =
          rotateIconSVG +
          `<span>Auto-Rotate: ${autoRotateEnabled ? "ON" : "OFF"}</span>`;
    };
    if (autoRotateToggle) {
  ...
  return (
    <div ref={containerRef} className="blackhole-container" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: 0 }}>
      ...
      <div id="controls" className="ui-panel">
